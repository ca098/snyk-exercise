...
class CachingService:
    def __init__(self):
        ...
        if self.redis_host is None or self.redis_port is None:
            print(
                f"No redis cache specified, assuming no cache. Host: {self.redis_host}, Port: {self.redis_port}"
            )
            self.r = None
    ...
    def put(
        self, key, value, ex_seconds=3600
    ):  # Default 1 hr? Not sure how often packages are updated...
        if self.r is None:
            return
        try:
            with self.lock:
